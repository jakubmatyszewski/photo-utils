...
def getDeviceModel():
    req = requests.get(GR_HOST + GR_PROPS)
    try:
        props = req.json()
...
def getPhotoList():
    req = requests.get(GR_HOST + PHOTO_LIST_URI)
    try:
        photoDict = req.json()
...
def fetchPhoto(photouri):
    try:
        if DEVICE == 'GR2':
            f = requests.get(GR_HOST+photouri)
        else: 
            f = requests.get(GR_HOST+PHOTO_LIST_URI+'/'+photouri)
        with open(PHOTO_DEST_DIR+photouri, "wb") as localfile:
            localfile.write(f.content)
        return True
    except Exception:
        return False

def shutdownGR():
    requests.post("http://192.168.0.1/v1/device/finish", json={})
